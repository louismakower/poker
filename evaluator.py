...
def beats(hand1: list[Card], hand2: list[Card]):
    """ Does hand1 beat hand2?"""
    rank1, scores1 = evaluation(hand1)
    rank2, scores2 = evaluation(hand2)
...
def winner(players: list, table_cards):
    hands = [player.get_best_hand(table_cards) for player in players]
    winning = 0

    for other_player in range(winning+1, len(hands)):
        if beats(hands[other_player], hands[winning]):
            winning = other_player
...
def test_compare_royal_flush_split_pot():
    ...
    assert beats(hand1, hand2) == 'split pot'
